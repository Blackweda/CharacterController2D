...
public class CharacterController2D : MonoBehaviour
{
    ...
    /// <summary>
    /// mask with all layers that the player in ladder
    /// </summary>
    public LayerMask ladderMask = 0;

    /// <summary>
    /// mask with all layers that the player in water
    /// </summary>
    public LayerMask waterMask = 0;

    /// <summary>
    /// mask with all layers that the player not should interact with
    /// </summary>
    public LayerMask nullMask = 0;
	...
	public bool isHalter { get { return collisionState.above; } }
    public bool isSuspended { get { return (collisionState.left || collisionState.right) && isFalling; } }
    ...
    void Update()//<============
    {
        ...
        {
              colliderUseableHeight = boxCollider.size.y * Mathf.Abs(transform.parent.transform.localScale.y) - (2f * skinWidth);
              colliderUseableWidth = boxCollider.size.x * Mathf.Abs(transform.parent.transform.localScale.x) - (2f * skinWidth);
        }
        else
        {
            colliderUseableHeight = boxCollider.size.y * Mathf.Abs(transform.localScale.y) - (2f * skinWidth);
            colliderUseableWidth = boxCollider.size.x * Mathf.Abs(transform.localScale.x) - (2f * skinWidth);
        }
	...
	private void primeRaycastOrigins( Vector3 futurePosition, Vector3 deltaMovement )
	{
        ...
        var scaledColliderSize = new Vector2();

        if (transform.parent != null)
        {
            scaledColliderSize = new Vector2(boxCollider.size.x * Mathf.Abs(transform.parent.transform.localScale.x), boxCollider.size.y * Mathf.Abs(transform.parent.transform.localScale.y)) / 2;
            scaledCenter = new Vector2(boxCollider.center.x * transform.parent.transform.localScale.x, boxCollider.center.y * transform.parent.transform.localScale.y);
        }
        else 
        {
            scaledColliderSize = new Vector2(boxCollider.size.x * Mathf.Abs(transform.localScale.x), boxCollider.size.y * Mathf.Abs(transform.localScale.y)) / 2;
            scaledCenter = new Vector2(boxCollider.center.x * transform.localScale.x, boxCollider.center.y * transform.localScale.y);
        }
	...
	private void moveHorizontally( ref Vector3 deltaMovement )
	{
        ...
        for (var i = 0; i < totalHorizontalRays; i++)
        {
            ...
            DrawRay(ray, rayDirection, Color.red);
            _raycastHit = Physics2D.Raycast(ray, rayDirection, rayDistance, nullMask & ~(platformMask & oneWayPlatformMask));
        ...
        for (var i = 0; i < totalHorizontalRays; i++)
        {
            ...
            if (isGoingRight)
                ray = new Vector2(initialRayOrigin.x-0.5f, initialRayOrigin.y + i * _verticalDistanceBetweenRays);
            else
                ray = new Vector2(initialRayOrigin.x+0.5f, initialRayOrigin.y + i * _verticalDistanceBetweenRays);

            DrawRay(ray, rayDirection, Color.blue);
        ...
        for (var i = 0; i < totalHorizontalRays; i++)
        {
            ...
            _raycastHit = Physics2D.Raycast(ray, rayDirection, rayDistance, waterMask);
	...
	private void moveVertically( ref Vector3 deltaMovement )
	{
        ...
        // if we are moving up, we should ignore the layers in ladderMask
        mask = ladderMask;

        isLadder = false;

        if(deltaMovement.x < 0)
        for (var i = 0; i < totalVerticalRays/2; i++)
        {
            var ray = new Vector2();
            ...
            for (var i = totalVerticalRays-1; i > totalVerticalRays / 2; i--)
            {
                var ray = new Vector2();
